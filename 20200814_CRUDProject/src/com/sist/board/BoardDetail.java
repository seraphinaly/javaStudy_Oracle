...
public class BoardDetail extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<h2>작성글 보기</h2>");
		out.println("<div class=row>");
		
		out.println("<table class=\"table table\">");
		//1번째줄
		out.println("<tr>");
		out.println("<td class=\"danger text-center\" width=25%>번호</td>");
		out.println("<td width=25% class=text-center>"+vo.getNo()+"</td>");
		out.println("<td class=\"danger text-center\" width=25%>작성일</td>");
		out.println("<td width=25% class=text-center>"+vo.getRegdate()+"</td>");
		out.println("</tr>");
		//2번째줄
		out.println("<tr>");
		out.println("<td class=\"danger text-center\" width=25%>이름</td>");
		out.println("<td width=25% class=text-center>"+vo.getName()+"</td>");
		out.println("<td class=\"danger text-center\" width=25%>조회수</td>");
		out.println("<td width=25% class=text-center>"+vo.getHit()+"</td>");
		out.println("</tr>");
		//3번째줄
		out.println("<tr>");
		out.println("<td class=\"danger text-center\" width=25%>제목</td>");
